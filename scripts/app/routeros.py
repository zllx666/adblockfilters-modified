    ...
    def generate(self, isLite=False):
        try:
            ...
            with open(fileName, "a") as f:
                f.write("#\n")
                if isLite:
                    f.write("# Title: AdBlock RouterOS Lite\n")
                    f.write("# Description: 适用于 RouterOS 的去广告合并规则，每 12 小时更新一次。规则源：%s。Lite 版仅针对国内域名拦截。\n" % (self.sourceRule))
                else:
                    f.write("# Title: AdBlock RouterOS\n")
                    f.write("# Description: 适用于 RouterOS 的去广告合并规则，每 12 小时更新一次。规则源：%s。\n" % (self.sourceRule))
                f.write("# Homepage: %s\n" % (self.homepage))
                ...
                for domain in blockList:
                    f.write("ip dns static add address=240.0.0.1 name=%s\n" % (domain))
                    f.write("ip dns static add address=:: name=%s\n" % (domain))

            if isLite:
                logger.info("adblock RouterOS Lite: block=%d" % (len(blockList)))
            else:
                logger.info("adblock RouterOS: block=%d" % (len(blockList)))
