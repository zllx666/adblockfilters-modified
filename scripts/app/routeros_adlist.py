...
class RouterOSAdList(APPBase):
    ...
    def __init__(self, blockList: List[str], unblockList: List[str], filterDict: Dict[str, str], filterList: List[str], filterList_var: List[str], ChinaSet: Set[str], fileName: str, sourceRule: str):
        super(RouterOSAdList, self).__init__(blockList, unblockList, filterDict, filterList, filterList_var, ChinaSet, fileName, sourceRule)

    def _get_max_bytes(self) -> int:
        raw = os.environ.get("ROUTEROSADLIST_MAX_BYTES")
        if not raw:
        ...
        logger.warning(f"Invalid ROUTEROSADLIST_MAX_BYTES={raw}, fallback to {self._DEFAULT_MAX_BYTES}")
        return self._DEFAULT_MAX_BYTES

    def generate(self, isLite=False):
        try:
            if isLite:
                logger.info("generate adblock RouterOS AdList Lite...")
                fileName = self.fileNameLite
                blockList = self.blockListLite
            else:
                logger.info("generate adblock RouterOS AdList...")
            ...
            try:
                with open(temp_entries, "w", encoding="utf-8", newline="\n") as f:
                    prelude = "0.0.0.0 localhost\n::1 localhost\n\n"
                    prelude_bytes = len(prelude.encode("utf-8"))
                    if prelude_bytes <= entry_budget:
                        f.write(prelude)
                        written_bytes += prelude_bytes
                    else:
                        truncated = True
                ...
                header = [
                    "#\n",
                    "# Title: AdBlock RouterOS AdList Lite\n" if isLite else "# Title: AdBlock RouterOS AdList\n",
                    "# Description: 适用于 RouterOS AdList 的去广告合并规则，每 12 小时更新一次。规则源：%s。Lite 版仅针对国内域名拦截。\n" % (self.sourceRule)
                    if isLite else
                    "# Description: 适用于 RouterOS AdList 的去广告合并规则，每 12 小时更新一次。规则源：%s。\n" % (self.sourceRule),
                    "# Homepage: %s\n" % (self.homepage),
            ...
            if truncated:
                logger.warning(
                    f"adblock RouterOS AdList{' Lite' if isLite else ''} truncated: "
                    f"keep={included_count}, total={len(blockList)}, max_bytes={max_bytes}"
                )
            elif isLite:
                logger.info("adblock RouterOS AdList Lite: block=%d" % (included_count))
            else:
                logger.info("adblock RouterOS AdList: block=%d" % (included_count))
